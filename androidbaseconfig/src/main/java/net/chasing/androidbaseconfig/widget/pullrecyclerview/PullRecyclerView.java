package net.chasing.androidbaseconfig.widget.pullrecyclerview;
...
import net.chasing.androidbaseconfig.widget.pullrecyclerview.manager.PullLinearLayoutManager;
import net.chasing.androidbaseconfig.widget.pullrecyclerview.manager.PullStaggeredGridLayoutManager;
import net.chasing.androidbaseconfig.widget.pullrecyclerview.manager.PullGridLayoutManager;
 ...
 */
public class PullRecyclerView extends RecyclerView implements Runnable {

    private Context mContext;

    private static PullRecyclerView rfView;
    ...
    public void setHeaderImage(ImageView headerImage) {
        ...
        headerImageHeight = PullRecyclerView.this.headerImage.getHeight();
        // 防止第一次拉伸的时候headerImage.getLayoutParams().height = 0
        if (headerImageHeight <= 0) {
            headerImageHeight = PullRecyclerView.this.headerImage.getLayoutParams().height;
        } else {
    ...
    public void run() {
        ...
        if (manager instanceof PullLinearLayoutManager) {
            // ListView布局
            ((PullLinearLayoutManager) manager).setOverScrollListener(mOverScrollListener);
        } else if (manager instanceof PullGridLayoutManager) {
            layoutGridAttach((PullGridLayoutManager) manager);
        } else if (manager instanceof PullStaggeredGridLayoutManager) {
            layoutStaggeredGridHeadAttach((PullStaggeredGridLayoutManager) manager);
        }
     ...
     *
     * @param manager {@link PullStaggeredGridLayoutManager}
     */
    private void layoutStaggeredGridHeadAttach(PullStaggeredGridLayoutManager manager) {
    ...
    /**
     * 给{@link PullGridLayoutManager}附加头部脚部和滑动过度监听
     *
     * @param manager {@link PullGridLayoutManager}
     */
    private void layoutGridAttach(final PullGridLayoutManager manager) {
        // GridView布局
        manager.setOverScrollListener(mOverScrollListener);
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == RecyclerView.SCROLL_STATE_IDLE && mLoadDataListener != null && !isLoadingData) {
            ...
            if (layoutManager instanceof PullGridLayoutManager) {
                lastVisibleItemPosition = ((PullGridLayoutManager) layoutManager).findLastVisibleItemPosition();
            } else if (layoutManager instanceof PullStaggeredGridLayoutManager) {
                int[] into = new int[((PullStaggeredGridLayoutManager) layoutManager).getSpanCount()];
                ((PullStaggeredGridLayoutManager) layoutManager).findLastVisibleItemPositions(into);
                lastVisibleItemPosition = findMax(into);
            } else {
                lastVisibleItemPosition = ((PullLinearLayoutManager) layoutManager).findLastVisibleItemPosition();
            }
